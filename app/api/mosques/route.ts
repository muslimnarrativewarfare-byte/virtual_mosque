...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!result.created) {
      return NextResponse.json(
        { id: result.mosque.id, message: "This mosque already exists. Existing listing returned." },
        { status: 200 }
      );
    }

    return NextResponse.json({ id: result.mosque.id, message: "Mosque created successfully." }, { status: 201 });
  } catch (error) {
    return NextResponse.json(
